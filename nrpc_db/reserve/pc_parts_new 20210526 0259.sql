...
-- Дата скрипта: 26.05.2021 2:59:34
...
--
-- Удалить функцию `changeAdmin`
--
DROP FUNCTION IF EXISTS changeAdmin;

--
-- Удалить функцию `createNewOrd`
--
DROP FUNCTION IF EXISTS createNewOrd;

--
-- Удалить функцию `getEmail`
--
...
--
-- Удалить процедуру `ordGetParts`
--
DROP PROCEDURE IF EXISTS ordGetParts;

--
-- Удалить процедуру `selectOrds`
--
DROP PROCEDURE IF EXISTS selectOrds;

--
-- Удалить процедуру `selectUsers`
--
...
--
-- Удалить процедуру `userOrds`
--
DROP PROCEDURE IF EXISTS userOrds;

--
-- Удалить функцию `modifyOrder`
--
DROP FUNCTION IF EXISTS modifyOrder;

--
-- Удалить таблицу `order_part`
--
...
--
-- Удалить функцию `checkSupply`
--
DROP FUNCTION IF EXISTS checkSupply;
...
--
-- Удалить представление `get_categories`
--
DROP VIEW IF EXISTS get_categories CASCADE;
...
--
-- Удалить представление `get_suppliers`
--
DROP VIEW IF EXISTS get_suppliers CASCADE;
...
--
-- Удалить представление `get_shops`
--
DROP VIEW IF EXISTS get_shops CASCADE;

--
-- Удалить процедуру `ordInfo`
--
...
--
-- Удалить представление `get_status`
--
DROP VIEW IF EXISTS get_status CASCADE;
...
--
-- Создать представление `get_status`
--
CREATE
VIEW get_status
AS
...
--
ALTER TABLE ord
ADD CONSTRAINT FK_ord_shop_id FOREIGN KEY (id_shop)
...
--
-- Создать представление `get_suppliers`
--
CREATE
VIEW get_suppliers
AS
...
--
-- Создать представление `get_categories`
--
CREATE
VIEW get_categories
AS
...
--
-- Создать функцию `checkSupply`
--
CREATE FUNCTION checkSupply (ord_id int, part_id int, ord_count int)
RETURNS tinyint(1)
BEGIN
  DECLARE supply int;
  DECLARE shop_id int;
...
ADD CONSTRAINT FK_order_part_ord_id FOREIGN KEY (id_order)
...
--
ALTER TABLE order_part
ADD CONSTRAINT FK_order_part_part_id FOREIGN KEY (id_part)
REFERENCES part (id) ON DELETE NO ACTION ON UPDATE NO ACTION;

DELIMITER $$

--
-- Создать функцию `modifyOrder`
--
...
--
-- Создать процедуру `selectOrds`
--
CREATE PROCEDURE selectOrds (ord_id varchar(255), login varchar(255) CHARSET utf8, shop_id int, status_id int)
BEGIN
  SELECT
    o.id,
    u.login,
...
(1, 'erius', 'Roge2003', 'Еро', 'Капралов', '89052592189', 'egor_kapralov2003@mail.ru', 'ул. Рогожский Поселок, д. 113', 1, '_', 2),
(2, 'WalrusYT', 'LubluMamy2003', 'Илья', 'Тат', '89630899425', 'taytsev@mail.ru', 'ул. Дружбы, д. 7', 0, '_', 0),
(3, 'arcwarden', 'pudgemom', 'Стэко', 'Слэйв', '89577366567', 'xrenxrenxrenxren@mail.ru', 'ул. Ращупкина, д. 21', 1, '_', 0),
...
(13, 'wawa', 'rdfg', 'ерог', 'карпов', '79052592199', '+79052592189', 'не скажу', 0, '_', 1);
...
(51, '32132', '123', 0.00, 1, 1, 'w', '32132.jpg');

-- 
-- Вывод данных для таблицы ord
--
INSERT INTO ord VALUES
(132, 1, 2, 2, '2021-05-24'),
(133, 1, 2, 2, '2021-05-24'),
(134, 1, 2, 2, '2021-05-24'),
(135, 1, 2, 2, '2021-05-24'),
(136, 1, 2, 2, '2021-05-24'),
(137, 1, 2, 2, '2021-05-24'),
(138, 1, 2, 2, '2021-05-24'),
(139, 1, 2, 2, '2021-05-25'),
(140, 1, 1, 4, '2021-05-25'),
(141, 1, 1, 2, '2021-05-26'),
(142, 1, 1, 4, '2021-05-26'),
(143, 1, 1, 2, '2021-05-26');
...
-- 
-- Вывод данных для таблицы order_part
--
